...
```{r simulation_roc_ratio,fig.width = 6,fig.height = 6}
dt.rcurve.gibbs <- dt.rcurve[Quantifier == 'Gibbs',]
dt.rcurve.boot <- dt.rcurve[Quantifier == 'Bootstrap',]

setnames(dt.rcurve.gibbs,old = c('oTPR'),new = 'oTPR.Gibbs')
setnames(dt.rcurve.boot,old = c('oTPR'),new = 'oTPR.Bootstrap')

dt.rcurve.boot <- 
  merge(dt.rcurve.boot,
        dt.rcurve.gibbs[,c('LibsPerGroup','Method','nFDR','oTPR.Gibbs')],
        by = c('LibsPerGroup','Method','nFDR'),all.x = TRUE,sort = FALSE)

dt.rcurve.boot[,round(mean(oTPR.Gibbs/oTPR.Bootstrap),1),by = c('LibsPerGroup','nFDR')]

ggplot(data = dt.rcurve.boot,aes(x = oFDR,y = (oTPR.Gibbs/oTPR.Bootstrap),
                                 group = interaction(Method),color = Method)) +
  facet_grid(rows = vars(LibsPerGroup),scales = 'free') +
  geom_line(linewidth = 0.5) +
  geom_point(aes(shape = as.factor(nFDR)),size = 3) +
  geom_vline(xintercept = c(0.01,0.05,0.1,0.15,0.2),linetype = 'dashed',linewidth = 0.25,color = 'gray') +
  geom_hline(yintercept = 1,linetype = 'solid',linewidth = 1,color = 'gray') +
  scale_x_continuous(limits = c(0,0.2)) +
  scale_color_viridis_d(direction = -1)  +
  theme_bw(base_size = bs,base_family = 'sans') +
  theme(panel.grid = element_blank(),
        ...
        axis.text = element_text(colour = 'black',size = bs),
        strip.text = element_text(colour = 'black',size = bs)) +
  labs(x = 'Observed Bootstrap FDR',y = 'Observed (Gibbs/Bootstrap) TPR',shape = 'Nominal FDR',color = 'Resamples')
```
