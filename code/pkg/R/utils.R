...
catchSalmonTPM <- function (paths, verbose = TRUE) 
{
...
catchKallistoTPM <- function(paths,verbose = TRUE){
  NSamples <- length(paths)
...
catchSalmon2 <- function(paths,verbose=TRUE,nboot.max=NULL)
  ...
  #	This version to test out effect of using few bootstrap samples
  #	Gordon Smyth
  #	Created 1 April 2018. Last modified 9 Dec 2023.
{
  NSamples <- length(paths)
  ...
  for (j in 1L:NSamples) {
    ...
    if(NBoot > 0L) {
      ...
      close(BootFileCon)
      dim(Boot) <- c(NTx,NBoot)
      if(!is.null(nboot.max)) {
        nboot.max <- as.integer(nboot.max)
        if(NBoot > nboot.max) {
          NBoot <- nboot.max
          Boot <- Boot[,1:NBoot]
        }
...
catchKallisto2 <- function(paths,verbose=TRUE,nboot.max=NULL)
  #	Read transcriptwise counts and bootstrap samples from kallisto output
  #	Use bootstrap samples to estimate overdispersion of transcriptwise counts
  #	Gordon Smyth
  #	Created 2 April 2018. Last modified 9 Dec 2023.
{
  NSamples <- length(paths)
